  ...
  function binz38Parser( html ) {
    ...
    var offers = entities.decode(
      removeWhitespace(
        $.load( html )( '.showcase_text' ).eq( 0 ).html()
          .replace( /<br><br>/g, ';;' ) // use double <br> as limiters for offers
          .replace( /<br>/g, '' )       // remove other <br>
          .replace( /<a.*<\/a>/g, '' )  // remove link at the end
      )
    )
    .split( ';;' )
    .slice( 1 );  // remove first item (date)
    ...
    _.each( offers, function ( offer ) {
      results.push(
        {
          title      : 'Menu ' + counter++,
          description: offer.replace( /<b>/g, '\n_CHF ' ).replace( /\s*<\/b>/g, '_' )
        }
      )
    } );
